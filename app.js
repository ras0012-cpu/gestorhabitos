...
const UI = (() => {
  ...
  const switchView = (v) => {
    ...
    if (v === 'crear') el('habito-fecha-puntual').value = _currentDate;
  };
      ...
      habitosDelDia.forEach(h => {
        ...
        card.innerHTML = `
          ...
          <div class="habito-actions">
            <button class="btn-completar ${h.stats.completadoHoy ? 'done-today' : ''}" ${h.stats.completadoHoy ? 'disabled' : ''}>
        ...
        card.querySelector('.btn-eliminar').onclick = () => { if(confirm('¿Borrar?')) API.eliminarHabito(h.id).then(renderHabitos); };
  ...
  const init = () => {
    ...
    el('habito-tipo').onchange = (e) => el('field-fecha-puntual').classList.toggle('hidden', e.target.value !== 'puntual');
